...
export default class CheckRunner {
  ...
  async runWorkflow(timeout: number = 15_000) {
    ...
    console.log("Fetched", response?.status === 200 ? response?.data?.map(car => ({id: car.model_id, name: car.name})) : response?.status);
    ...
    fs.writeFileSync("models.json", JSON.stringify(response?.data));
